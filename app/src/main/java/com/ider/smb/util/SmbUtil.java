...
public class SmbUtil {
	...
	public SmbUtil(MainActivity main, Handler handler) {
	...
	private class ConnectHost implements Runnable{
		...
		public void run() {
			...
			if (ping(host)){
				  ...
				  smbinfo.mFile = new File(EnumConstent.mDirSmb+"/"+subnetaddress);				  
	...
	public void searchSmb(){
		...
		Log.i("SmbUtil","vectorlist.size()"+vectorlist.size());
		for(int m = 0;m < vectorlist.size();m++){
			Vector<InetAddress> vector = vectorlist.get(m);
			Log.i("SmbUtil","vector.size()"+vector.size());
	...
	public static String mount(String smbpath,FileInfo smbinfo){
		...
		if (smbinfo.getMMountpoint()!=null){
			...
			mountPoint = EnumConstent.mDirSmbMoutPoint+"/"+sf.format(new Date());
		}
	...
	public static boolean creatMountPoint(String path){
		try{
			File root_smb = new File(EnumConstent.mDirSmbMoutPoint);
		if(!root_smb.exists()){
			if(!root_smb.mkdirs()){
				return false;	
			}else{
