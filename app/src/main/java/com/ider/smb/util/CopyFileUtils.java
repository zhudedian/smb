...
public class CopyFileUtils { 
	...
	public static boolean is_not_free_space = false;
    ...
    public int getfilenum(ArrayList<FileInfo> multi_path){
    	...
    	for(int i = 0; i < multi_path.size(); i ++){
    		if((multi_path.get(i).mIsDir) && (multi_path.get(i).mFile.isDirectory())){
    ...
    public boolean ishavefreespace(File sourcefile, File targetfile){
		...
		pathError = false;
    	if(targetfile.getPath().startsWith(EnumConstent.mDirSmbMoutPoint)){
